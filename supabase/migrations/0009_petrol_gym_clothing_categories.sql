-- Add new default expense categories: Petrol, Gym, Clothing.
...
begin
  ...
  insert into categories (user_id, name, icon, kind, is_default) values
    ...
    (new.id, 'Petrol',            'petrol',            'expense', true),
    (new.id, 'Gym',               'gym',               'expense', true),
    (new.id, 'Clothing',          'clothing',          'expense', true),
